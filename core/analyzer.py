...
def analyze_portfolio(
    ...
    if mode == "morning":
        format_header = (
            "🚨 OUTPUT-REGEL (NEU ab 2026-05-21):\n\n"
            "TOOL-FLOW:\n"
            "1. recommend_entry für JEDES A+ Setup (Conv ≥3/5) — 0 bis 5 Calls.\n"
            "   entry_price = struktureller Ideal-Preis (Limit-Buy, KANN/SOLL < live_price sein).\n"
            "   accumulation_zone, support_bounce, pullback_ma → recommend_entry mit setup_type, NICHT als Watch.\n"
            "2. set_watch_levels NUR mit Defense-Watches für OFFENE Positionen + echte breakout_confirm.\n"
            "   Bei 0 offenen Positionen + keine breakout_confirm-Triggers nötig → `levels: []` ist KORREKT.\n"
            "   accumulation_zone-Watches für Entry-Suche = VERBOTEN.\n\n"
            "TEXT-OUTPUT MUSS mit GENAU einer dieser Zeilen beginnen UND ENDEN:\n"
            "  • `TICKER | €X (+/-X%) | SL/TP | HALTEN/CLOSE/...` (für offene Position)\n"
            "  • `TICKER | Entry €X | SL €X | TP €X | Size €X | Conv X/5 | These ...` (für JEDES A+ Setup, eine Zeile pro Rec, durch \\n getrennt)\n"
            ...
            "❌ FALSCH (Sonnet's Drift heute 08:41 — STOP):\n"
            "    set_watch_levels: [MBG accumulation_zone 49.75, BAS accumulation_zone 51.10, RWE accumulation_zone 56.00, NDX1 accumulation_zone 43.00, ...] (12 Watch-Levels)\n"
            "    Text: '' (leer)\n\n"
            "✅ RICHTIG:\n"
            "    recommend_entry: MBG.DE | entry_price=49.20 | sl=48.50 | tp=[51.50,53.00] | conv=3 | setup_type=support_bounce | thesis=...\n"
            "    recommend_entry: BAS.DE | entry_price=51.10 | sl=50.30 | tp=[52.80,54.00] | conv=3 | setup_type=support_bounce | thesis=...\n"
            "    set_watch_levels: levels=[]\n"
            "    Text: 'MBG.DE | Entry €49.20 | SL €48.50 | TP €51.50 | Size €190 | Conv 3/5 | These MA-Reclaim\\nBAS.DE | Entry €51.10 | SL €50.30 | TP €52.80 | Size €200 | Conv 3/5 | These Squeeze-Support'\n\n"
            "Nach den Pflicht-Zeilen: EOF. Kein Makro-Kommentar, kein 'aber beachte...', kein Watch-Level-Recap.\n\n"
        )
    elif mode == "opening":
        format_header = (
