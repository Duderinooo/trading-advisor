...
1. **Read-only on portfolio state.** Never write `portfolio.json` or any portfolio-state table (bot owns them under `core.portfolio.portfolio_lock`; racing the lock = corruption). **One sanctioned exception (2026-06-16):** the proposed-trade Accept/Cancel/Fire buttons enqueue into the `web_commands` SQLite table via `enqueueWebCommand` ([lib/db.ts](lib/db.ts) write connection + [app/api/commands/route.ts](app/api/commands/route.ts)). The bot drains that queue and performs the actual portfolio write under its lock. So: web writes *commands*, never *state*. Any other write feature still routes through Telegram or this queue — never direct fs / portfolio tables.
