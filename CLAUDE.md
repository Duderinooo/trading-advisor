...
4. **Whole shares only**: user trades ganze Stücke on TR (operational preference — TR technically supports Bruchstücke but user does not use them). `shares` stays `float`-typed for legacy compat but must always be integer-valued. Entry, ADD, and partial-TP all floor to int via `float(int(...))`. Never persist `5.7` shares — Telegram alert would be unactionable ("verkauf 2.5 Stk" on TR not possible).
...
- **TP1 lock-in (no sell)**: TP1 hit on a multi-TP rec triggers BE-shift (entry + fee-buffer) + 1.5×ATR trailing — full position runs to TP2 or trail-stop. Default `PARTIAL_TP_FRACTION = 0` (2026-05-25 change: partial-sell removed because €1/side TR fee burned 25-50% of partial gross at €1k capital). Set fraction > 0 to re-enable partial-sell behavior (will sell `int(shares × fraction)` + same BE/trail on remainder).
- **Final-TP setup-type aware** (2026-05-25): final TP hit branches on `setup_type`:
  - **Trend-family** (`TREND_FOLLOW_SETUPS` in `config/execution.py`: breakout_resistance, flag_continuation, earnings_drift, pre_breakout_squeeze) → no sell. SL ratcheted to TP − 0.5%, trail tightened to `TRAIL_TIGHTEN_ATR_MULT_FINAL_TP × ATR` (default 1.0×). Position runs to trail-stop → captures monster outliers when momentum continues.
  - **Swing-family** (mean_reversion, support_bounce, reversal_oversold, gap_fill, pullback_ma20/50) → close full at TP. TPs are analytical resistance-cluster targets; typically reverse → take profit at target.
